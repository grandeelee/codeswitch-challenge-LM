...
# load data
data = load_data(args)
args.log_interval = (len(data['penn']['train']) // args.batch_size) // 10
...
n_updates_total = (len(data['penn']['train']) // args.batch_size) * args.epochs
...
def run_epoch(train_gen):
    ...
    batch = 0
    n_words = 0
    epoch_size = (len(data['penn']['train']) // args.batch_size)
    # for _ in range(10):
...
if __name__ == '__main__':
    ...
    try:
        for epoch in range(args.epochs):
            ...
            train_iterator = get_iterator('penn', 'train')
            run_epoch(train_iterator)
            valid_iterator = get_iterator('penn', 'valid')
    ...
    # Run on test data.
    test_iterator = get_iterator('penn', 'test')
