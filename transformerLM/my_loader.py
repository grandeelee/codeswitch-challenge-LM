...
def load_mono_data(params, data):
    ...
    for splt in ['train', 'valid', 'test']:
        ...
        mono_data = load_binarized(params.mono_dataset['penn'][splt], params)
...
def check_data_params(params):
    ...
    params.mono_dataset = {
        'penn': {
            splt: os.path.join(params.data, '{}.{}.pth'.format(splt, 'txt'))
            for splt in ['train', 'valid', 'test']  #
        }
    }
...
def load_data(params):
    ...
    for data_set in data['penn'].keys():
        logger.info(
            '{: <18} - {: >5} - {: >12}:{: >10}'.format('Monolingual data', data_set, 'penn', len(data['penn'][data_set])))
