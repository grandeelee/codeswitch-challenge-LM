...
class LMModel(nn.Module):
    ...
    def forward(self, x):
        l, h = self.transformer(x)
        previous_lm_logits = self.decoder(h)
        last_lm_logits = self.decoder(l)
        if self.return_probs:
            last_lm_logits = F.softmax(last_lm_logits + self.pos_emb_mask, dim=-1)
        return last_lm_logits, previous_lm_logits
